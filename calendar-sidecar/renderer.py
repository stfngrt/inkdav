...
renderer.py
===========
Renders a 7-column week grid as a 1-bit PNG optimised for e-ink displays.
...
def render_week(
    ...
    # ── 1. Header ─────────────────────────────────────────────────────────────
    draw.rectangle([0, 0, width, HEADER_H - 1], fill=DGREY)

    for col in range(7):
        day = week_start + timedelta(days=col)
        x0  = TIME_AXIS_W + col * col_w
        x1  = x0 + col_w - 1

        if day == today and today_highlight:
            draw.rectangle([x0, HEADER_H, x1, height - 1], fill=LGREY)
            draw.rectangle([x0, HEADER_H, x1, height - 1], outline=MGREY, width=1)
        if col > 0:
            draw.line([x0, 0, x0, height], fill=MGREY, width=1)

        label = f"{DAY_NAMES[col]} {day.day:02d}.{day.month:02d}."
        tw    = draw.textlength(label, font=FONT_DAY)
        draw.text((x0 + (col_w - tw) / 2, 5), label, font=FONT_DAY, fill=WHITE)

    # ── 2. Legend ─────────────────────────────────────────────────────────────
    draw.rectangle([0, HEADER_H, width, HEADER_H + LEGEND_H - 1], fill=240)
    draw.line([0, HEADER_H + LEGEND_H - 1, width, HEADER_H + LEGEND_H - 1],
              fill=MGREY, width=1)
    ...
    # ── 3. Collect events per day ─────────────────────────────────────────────
    day_events: dict[int, list[CalEvent]] = {i: [] for i in range(7)}
    for _, (_, events) in events_by_cal.items():
        for ev in events:
            offset = (ev.start.date() - week_start).days
            if 0 <= offset < 7:
                day_events[offset].append(ev)

    # ── 4. All-day strip ──────────────────────────────────────────────────────
    draw.line([TIME_AXIS_W - 1, allday_top, TIME_AXIS_W - 1, height],
              fill=MGREY, width=1)
    draw.line([0, allday_top + ALLDAY_H, width, allday_top + ALLDAY_H],
              fill=MGREY, width=1)

    for col in range(7):
        allday_evs = [e for e in day_events[col] if e.all_day]
        if not allday_evs:
            continue
        x0  = TIME_AXIS_W + col * col_w + PADDING
        cw  = col_w - PADDING * 2
        ev  = allday_evs[0]
        ...
        draw.rectangle([x0, allday_top + 1, x0 + cw, allday_top + ALLDAY_H - 2],
                       fill=grey, outline=BLACK)
        draw.text((x0 + 2, allday_top + 2), lbl, font=FONT_EVENT,
                  fill=WHITE if grey < 128 else BLACK)

    # ── 5. Time axis + hour grid lines ────────────────────────────────────────
    for h in range(time_start_hour, min(time_end_hour + 1, 25)):
        y = grid_top + int((h - time_start_hour) * px_per_hour)
        if y >= height:
            break
        draw.line([TIME_AXIS_W, y, width, y], fill=MGREY, width=1)
        draw.text((1, y - 9), f"{h:02d}", font=FONT_TIME, fill=MGREY)

    # ── 6. "Now" indicator on current-week today column ──────────────────────
    from datetime import datetime
    now = datetime.now()
    now_frac = now.hour + now.minute / 60
    today_col = (today - week_start).days
    if (0 <= today_col < 7
            and time_start_hour <= now_frac < time_end_hour):
        ny  = grid_top + int((now_frac - time_start_hour) * px_per_hour)
        tx0 = TIME_AXIS_W + today_col * col_w
        draw.line([tx0, ny, tx0 + col_w, ny], fill=BLACK, width=2)
        draw.ellipse([tx0 - 3, ny - 3, tx0 + 3, ny + 3], fill=BLACK)

    # ── 7. Timed events ───────────────────────────────────────────────────────
    for col in range(7):
        timed_evs = sorted(
            [e for e in day_events[col] if not e.all_day],
        ...
        for ev in timed_evs:
            ...
            vis_start = max(start_frac, time_start_hour)
            vis_end   = min(end_frac,   time_end_hour)
            if vis_end <= vis_start:
                continue

            y0 = grid_top + int((vis_start - time_start_hour) * px_per_hour)
            y1 = grid_top + int((vis_end   - time_start_hour) * px_per_hour) - 1
            if y1 - y0 < EVENT_MIN_H:
                y1 = y0 + EVENT_MIN_H

            grey = _event_fill(_hex_to_grey(ev.color))
            _draw_timed_block(draw, ev, x0, y0, y1, cw, grey)

    # ── 8. Dither to 1-bit ────────────────────────────────────────────────────
    return img.convert("1", dither=Image.Dither.FLOYDSTEINBERG).convert("L")


# ── Event block renderers ─────────────────────────────────────────────────────
...
def _load_events(path: str) -> dict[str, tuple[str, list[CalEvent]]]:
    """
    Parse a JSON calendar file into the events_by_cal dict expected by render_week.
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Render a week calendar PNG from JSON event data.",
    )
